...
Twinkle is a Natural Language Processing (NLP) model capable of processing a growing database of billions of tweets and finding the most similar tweet in real-time as the user is typing a new tweet. This model computes similarities by using a hashing vectorizer and a Locality Sensitive Hashing Forest to achieve sub-linear scaling of computation time.
...
When we adopt a tool we also adopt the management philosophy embedded in that tool. Twitter users enjoy practicing conciseness and expressivity. Twitter is uniquely equipped with refined super powers to quickly spread information and host effective conversations; improving our opinions, actions and integrity. The most cutting edge social teachings are now happening on Twitter at light speed and hashtags are advancing the evolution of society beyond traditional expectations.
